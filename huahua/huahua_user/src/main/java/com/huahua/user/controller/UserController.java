...
public class UserController {
	...
	public Result delete(@PathVariable String id ){
		...
		if(StringUtils.isEmpty(header)){
			return new Result(false,StatusCode.LOGIN_ERROR,"登录有误，请返回重新登录");
		}
		//Bearer 也为空
		...
		//校验claims不能为空
		if (null == claims){
			return new Result(false,StatusCode.LOGIN_ERROR,"登录异常");
		}
		//判断是否具有admin权限
	...
	public Result login(@RequestBody User user){
		...
		if(null != respUser){
			...
			//JWT前端与后端访问的唯一标识 验证token 否则返回登录
			//吧token返回给前段（身份唯一标识）
			...
			return new Result(false,StatusCode.LOGIN_ERROR,"登陆信息有误");
		}
