...
public class UserService {
    ...
    public void sendsms(String mobile) {
		...
		//生成验证码
		String moblieCode = RandomUtil.randomNumbers(6);
		//往redis存放验证码 同时设置验证码的超时时间
		stringRedisTemplate.opsForValue().set("smsCode_"+mobile,moblieCode,5, TimeUnit.MINUTES);
		//通知rabbitmq发送短信 同时将手机号和验证码发送 --》 消息队列
		Map<String,String> map = new HashMap<>();
		map.put("mobile",mobile);
		map.put("code",moblieCode);
		//发送队列消息
