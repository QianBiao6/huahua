...
public class SpitService {
    ...
    public Boolean updateThumbup(String spitId) {
        String userid = "1";
        Spit spit = (Spit)redisTemplate.opsForValue().get("spit_" + spitId + userid);//评论id+用户id
        if(null == spit){//如果缓存不存在 可以点赞
            Query query = new Query();//创建查询语句
            query.addCriteria(Criteria.where("_id").is(spitId));
            Update update = new Update();
            update.inc("thumbup",1);
            mongoTemplate.updateFirst(query,update,"spit");
            redisTemplate.opsForValue().set("spit_" + spitId + userid,query);
            return true;
        }else{//缓存存在 不能点赞
    ...
    public Page<Spit> search(Integer page, Integer size) {
        PageRequest pageRequest = PageRequest.of(page - 1, size);
        return spitDao.search(pageRequest);
    }
